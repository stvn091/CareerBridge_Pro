...
{{
  ...
  "highlighted_resume_paragraph": "<single most relevant paragraph from the CV>",
  "critique_reason": "<one sentence explaining why that paragraph needs work or is strong>"
}}
...
"""
            ...
            try:
                ...
                st.session_state.audit_data = json.loads(raw_output)
    ...
    if st.session_state.audit_data:
        ...
        with res_left:
            st.markdown(f"<h4 style='color:{TEXT}'>📝 Resume Snapshot</h4>", unsafe_allow_html=True)

            # Show the highlighted paragraph from the CV
            highlight = data.get("highlighted_resume_paragraph", "No paragraph extracted.")
            critique  = data.get("critique_reason", "")

            st.markdown(f'<div class="highlight-box">{highlight}</div>', unsafe_allow_html=True)
            if critique:
